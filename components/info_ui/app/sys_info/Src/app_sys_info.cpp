...
void app_sys_info::draw_interface() {
    ...
    lv_obj_align(author_img_obj, LV_ALIGN_BOTTOM_RIGHT, 5, 13);
    lv_obj_set_size(author_img_obj, ((lv_img_dsc_t*)this->_author_img)->header.w, ((lv_img_dsc_t*)this->_author_img)->header.h);

    
    //------------------------------绘制信息------------------------------
    lv_obj_t* sys_label = lv_label_create(this->_app_panel);
    lv_label_set_text(sys_label, "INFO UI");
    lv_obj_set_style_text_font(sys_label, &lv_font_montserrat_20, 0);
    lv_obj_align(sys_label, LV_ALIGN_TOP_LEFT, -8, -10);

    //------------------------------作者信息------------------------------
    lv_obj_t* author_label = lv_label_create(this->_app_panel);
    lv_label_set_text(author_label, "By :\n"
                                    "Jayden-Lee\n"
                                    "Matr1x-Yu\n");
    lv_obj_set_style_text_font(author_label, &lv_font_montserrat_10, 0);
    lv_obj_align(author_label, LV_ALIGN_BOTTOM_LEFT, -8, 25);
}
